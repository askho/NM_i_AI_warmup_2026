...
from policies.optimized_multi_bot import BotIntent, OptimizedMultiBotPolicy, Target, _build_reservations, _need_after_carried
...
def test_need_after_carried_avoids_over_pick():
    need = Counter({"apple": 2, "milk": 1})
    bots = [
        {"id": 0, "inventory": ["apple"]},
        {"id": 1, "inventory": ["milk", "eggs"]},
        {"id": 2, "inventory": []},
    ]
    assert _need_after_carried(need, bots) == Counter({"apple": 1})
