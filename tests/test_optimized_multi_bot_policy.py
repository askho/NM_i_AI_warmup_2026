...
from policies.optimized_multi_bot import BotIntent, OptimizedMultiBotPolicy, Target, _build_reservations, _need_after_inventory
...
def test_need_after_inventory_subtracts_carried_items():
    need = Counter({"apple": 2, "banana": 1})
    bots = [
        {"id": 0, "inventory": ["apple"]},
        {"id": 1, "inventory": ["banana", "pear"]},
    ]
    adjusted = _need_after_inventory(need, bots)
    assert adjusted == Counter({"apple": 1})
