...
def test_idle_bot_reservation_stays_at_current_position():
    controller = DummyController()
    bots = [{"id": "0", "position": {"x": 3, "y": 3}, "inventory": []}]
    intents = {"0": BotIntent(mode="IDLE", targets=[])}
    reservations = _build_reservations(intents, bots, controller, horizon=3)
    assert reservations["0"] == {0: (3, 3), 1: (3, 3), 2: (3, 3), 3: (3, 3)}
